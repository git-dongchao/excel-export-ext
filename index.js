...
exports.execute = function(config, filepath, fn){
    JSZip.loadAsync(templateXLSX, {
        base64: true,
        checkCRC32: false
    }).then(function (xlsx) {
        doExecute(config, filepath, xlsx, fn);
    });
};
...
var doExecute = function(config, filepath, xlsx, fn) {
    ...
    // Step2：打开文件流
    var outTempfile = "temp/" + guid.create() + ".xml";
    var writeStream = fs.createWriteStream(outTempfile);

    // Step3：写入页头
    writeStream.write(sheetFront);

    shareStrings = new SortedMap(undefined, undefined, function (a, b) {
        var aType = typeof a;
        var bType = typeof b;
    ...
    var colStyleIndex;
    var colsHeader = '<x:row r="1" spans="1:' + colsLength + '">';
    var colsWidth = "";
    for (k = 0; k < colsLength; k++) {
        colStyleIndex = cols[k].captionStyleIndex || 0;
        colsHeader += addStringCol(getColumnLetter(k + 1) + 1, cols[k].caption, colStyleIndex);
        if (cols[k].width) {
    ...
    // Step4：写入列宽
    if (colsWidth !== "") {
        writeStream.write('<cols>');
        writeStream.write(colsWidth);
        writeStream.write('</cols>');
    }

    // Step5：开始写入页
    writeStream.write("<x:sheetData>");

    // Step6：写入列头
    writeStream.write(colsHeader);
    ...
    writeStream.on('finish', function () {
        writeStream.destroy();
        var readStream = fs.createReadStream(outTempfile);
        xlsx.file(sheet.name, readStream);
        xlsx.generateNodeStream({base64: false, compression: "DEFLATE"})
